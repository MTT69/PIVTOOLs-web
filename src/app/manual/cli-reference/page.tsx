...
export default function CLIReferencePage() {
  const commands: CommandCardProps[] = [
    ...
    {
      command: 'pivtools-cli init-settings',
      description: 'Create the calibration settings sidecar for a source',
      options: [
        { flag: '--source', description: 'Calibration source directory. The sidecar is written to <source>/calibration/settings.yaml' },
      ],
      examples: [
    ...
    },
    {
      ...
      options: [
        ...
        { flag: '--image-size', description: 'Image size as "W H". Skips loading a frame entirely, for sources with no calibration images on disk' },
        { flag: '--image-format', description: 'Calibration image filename pattern' },
      ...
      examples: [
        ...
        'pivtools-cli scale-factor --px-per-mm 20.5 --origin 100 900 --image-size 2560 1600',
      ],
      link: '/manual/planar-calibration#cli',
    },
    {
      ...
      options: [
        ...
        { flag: '--source', description: 'Calibration source directory' },
        { flag: '--dt', description: 'Time between frames (s)' },
        { flag: '--z-world', description: 'Light-sheet plane Z in mm (default 0.0, which is the calibration-board plane)' },
        { flag: '--tilt-x / --tilt-y', description: 'Sheet tilt in radians, not degrees. The plane is Z = z_world + X*tan(tilt_y) + Y*tan(tilt_x)' },
      ...
      examples: [
        ...
        'pivtools-cli apply-calibration --board charuco --uncalibrated-dir <dir> --calibrated-dir <dir>',
    ...
    },
    {
      ...
      options: [
        ...
        { flag: '--uncalibrated-dir-cam1', description: 'Explicit camera-1 uncalibrated vector directory' },
        { flag: '--uncalibrated-dir-cam2', description: 'Explicit camera-2 uncalibrated vector directory' },
        ...
        { flag: '--z-world', description: 'Light-sheet plane Z in mm (default: the self-calibration value stored in the record, else 0.0)' },
        { flag: '--tilt-x', description: 'Sheet tilt about X in radians, not degrees' },
        { flag: '--tilt-y', description: 'Sheet tilt about Y in radians, not degrees' },
      ],
      examples: [
        'pivtools-cli apply-stereo --board charuco --camera-pair 1,2 --all-paths',
        'pivtools-cli apply-stereo --board stepped --all-paths',
      ],
      link: '/manual/stereo-calibration#cli',
    },
    {
    ...
    },
    {
      ...
      description: 'Bake the multi-camera global frame (datum + overlap from the calibration settings sidecar) into each model',
      options: [
    ...
    },
    {
      ...
      description: 'Merge multi-camera vector fields with blended overlaps',
      options: [
    ...
    },
    {
      ...
      options: [
        { flag: '--camera', short: '-c', description: 'Camera number (default: all)' },
        { flag: '--type-name', short: '-t', description: 'instantaneous only. Ensemble PIV already writes means and Reynolds stresses directly, so the flag rejects it.' },
        { flag: '--source-endpoint', short: '-s', description: 'regular | merged | stereo' },
          ...
          <Section title="Quick Reference" icon={<Terminal size={28} />} id="quick-reference">
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6">
              <p className="text-yellow-800 text-sm">
                <strong>Both apply commands need to be told which directories to work on.</strong>{' '}
                <code className="bg-yellow-100 px-1 rounded">apply-calibration</code> and{' '}
                <code className="bg-yellow-100 px-1 rounded">apply-stereo</code> exit with an error unless you pass
                either <code className="bg-yellow-100 px-1 rounded">--all-paths</code>, which derives every base path
                from the config the way the GUI does, or an explicit input and output directory pair. There is no
                default, so a bare invocation will not run.
              </p>
            </div>
            <div className="bg-gray-900 rounded-xl p-6 mb-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm font-mono">
                ...
                <div>
                  ...
                  <div className="text-green-400">pivtools-cli init-settings</div>
...
pivtools-cli video --data-source stereo -v uz`}</CodeBlock>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                ...
                <CodeBlock>{`# Stepped-board detection is GUI-only: it needs interactive
              ...
              <table className="min-w-full bg-white rounded-lg overflow-hidden shadow-sm border border-gray-200">
                ...
                <tbody className="divide-y divide-gray-100">
                  {[
                    ...
                    { var: 'OMP_NUM_THREADS', desc: 'Ignored by the instantaneous and ensemble pipelines, which overwrite it at import from processing.omp_threads. Set that config key instead. Still honoured by tools that do not go through those pipelines.' },
                    { var: 'PIV_CAMERA', desc: 'Restrict a run to one camera. Set by the --camera flag. Process-global, so it must never be set in the long-lived GUI server process, where every later request would silently flip to single-camera mode.' },
                  ].map((item, i) => (
                    <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
